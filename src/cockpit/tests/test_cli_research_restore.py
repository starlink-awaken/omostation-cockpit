...
def test_research_help_includes_restore_option(monkeypatch, capsys):
...
def test_cmd_research_restore_requires_ids(monkeypatch):
    ...
    code = cli.cmd_research_restore(argparse.Namespace(restore=[]))
...
def test_cmd_research_restore_reports_result(monkeypatch):
    capture = Console(record=True, force_terminal=True, width=140)
    monkeypatch.setattr(cli, "console", capture)
    monkeypatch.setattr(cli, "err", capture)
    mock = MockDataAccess()
    mock.restore_research = lambda ids: ([4, 5], [9])
    monkeypatch.setattr(cli, "get_data_access", lambda: mock)

    code = cli.cmd_research_restore(argparse.Namespace(restore=[4, 5, 9]))
...
def test_cmd_research_restore_all_missing(monkeypatch):
    ...
    monkeypatch.setattr(cli, "err", capture)
    mock = MockDataAccess()
    mock.restore_research = lambda ids: ([], [1, 2])
    monkeypatch.setattr(cli, "get_data_access", lambda: mock)

    code = cli.cmd_research_restore(argparse.Namespace(restore=[1, 2]))
...
def test_cmd_research_restore_all_success(monkeypatch):
    capture = Console(record=True, force_terminal=True, width=140)
    monkeypatch.setattr(cli, "console", capture)
    monkeypatch.setattr(cli, "err", capture)
    mock = MockDataAccess()
